<?php
    ...
    $user = $_POST['inputData'];
    $date = $_POST['inputDate'];

    $table = "main_address_info";
    $sql = "SELECT * FROM $database_name.$table WHERE responsible LIKE '%$user%' ORDER BY cep";
    ...
    if ($rows) {
        foreach ($rows as $row) {
            ...
            echo  "<td class='border-collapse border-2 border-orange-700 text-blue-600 py-2 px-2 text-center'>
                <a href='' onclick=\"sendDataForDeletion('" . $row['entity_id'] . "," . $table . "')\"><i class=\"fa-solid fa-circle-info\"></i>i</a></td>\n";
            echo  "<td class='border-collapse border-2 border-orange-700 text-green-600 py-2 px-2 text-center'>
                <a href='' onclick=\"sendDataForDeletion('" . $row['entity_id'] . "," . $table . "')\"><i class=\"fa-solid fa-pen-to-square fa-lg\"></i>k</a></td>\n";
