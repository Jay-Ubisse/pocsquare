...
if ($rows) {
    foreach ($rows as $row) {
        ...
        echo  "<td class='border-collapse border-2 border-orange-700 py-2 px-5'>" . $row['neighborhood_locality']  . "</td>\n";
        echo  "<td class='border-collapse border-2 border-orange-700 py-2 px-5 text-center'><a href='#' 
        onclick=\"sendInfoForEdit('" . "neighborhood-data" . "," . $row['id'] . "," . $row['district'] . "," . $row['admin_post'] . "," . $row['neighborhood_locality'] . "')\" class='bg-green-600 px-3 py-1 rounded-sm text-white'>Editar</a><a href='#' class='bg-red-600 px-3 py-1 rounded-sm text-white ml-2'>Eliminar</a></td>\n";
        echo  "</tr>";
    }
} else {
