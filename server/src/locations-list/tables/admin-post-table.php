...
if ($rows) {
    foreach ($rows as $row) {
        ...
        onclick=\"sendInfoForEdit('" . "admin-post-data" . "," . $row['id'] . "," . $row['district'] . "," . $row['admin_post'] . "')\" class='bg-green-600 px-3 py-1 rounded-sm text-white'>Editar</a><a href='#' class='bg-red-600 px-3 py-1 rounded-sm text-white ml-2'>Eliminar</a></td>\n";
        echo  "</tr>";
    }
} else {
